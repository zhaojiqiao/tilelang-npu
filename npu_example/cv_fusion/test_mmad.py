...
parser.add_argument("--n", type=int, default=2048, help="Matrix N dimension")
parser.add_argument("--k", type=int, default=2048, help="Matrix K dimension")
...
c = torch.zeros(M, N).half().npu()
d = torch.randn(M, N).half().npu()
tmp = torch.zeros(M, N).half().npu()
