...
void main_kernel(GM_ADDR A_handle, GM_ADDR B_handle, GM_ADDR C_handle) {
  ...
  if (pid < 8192) {
    auto pid =
        tl::ascend::thread_block_swizzle<16384, 16384, 16384, 128, 256, 3, 0>(
            pid);
    ...
    for (int k = 0; k < 64; ++k) {
      AscendC::WaitFlag<AscendC::HardEvent::M_MTE2>((k % 2));
      {
        ...
        tl::ascend::copy_gm_to_l1<half, 16384, 16384, 128, 256>(
            A_L1_[((k % 2) * 32768)], A[(((pid / 64) * 2097152) + (k * 256))]);
      }
      {
        auto B_L1_ = B_L1.Get<half>();
        tl::ascend::copy_gm_to_l1<half, 16384, 16384, 256, 256>(
            B_L1_[((k % 2) * 65536)], B[((k * 4194304) + ((pid % 64) * 256))]);
      }
      ...
      for (int kk = 0; kk < 4; ++kk) {
        ...
        AscendC::WaitFlag<AscendC::HardEvent::M_MTE1>((kk % 2));
        {
          ...
          tl::ascend::copy_l1_to_l0a<half, 128, 256, 128, 64>(
              A_L0_[((kk % 2) * 8192)],
              A_L1_[(((k % 2) * 32768) + (kk * 8192))]);
        }
        {
          auto B_L1_ = B_L1.Get<half>();
          auto B_L0_ = B_L0.Get<half>();
          tl::ascend::copy_l1_to_l0b<half, 256, 256, 64, 256>(
              B_L0_[((kk % 2) * 16384)],
    ...
    AscendC::WaitFlag<AscendC::HardEvent::M_FIX>(0);
    {
      ...
      tl::ascend::copy_l0c_to_gm<float, half, 128, 256, 16384, 16384>(
          C[(((pid / 64) * 2097152) + ((pid % 64) * 256))], C_L0_[0]);
    }
...
extern "C" void call(uint8_t *A_handle, uint8_t *B_handle, uint8_t *C_handle,
                     aclrtStream stream) {
  main_kernel<<<8200, nullptr, stream>>>(A_handle, B_handle, C_handle);
}