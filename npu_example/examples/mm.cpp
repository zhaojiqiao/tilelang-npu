...
void main_kernel( GM_ADDR A_handle,  GM_ADDR B_handle,  GM_ADDR C_handle) {
  ...
  cid = tl::ascend::thread_block_swizzle<16384, 16384, 16384, 128, 256, 3, 0>(cid);
  ...
  for (int k = 0; k < 64; ++k) {
    AscendC::WaitFlag<AscendC::HardEvent::M_MTE2>((k % 2));
    {
      ...
      tl::ascend::copy_gm_to_l1<half, layout::RowMajor, layout::zN, 16384, 16384, 128, 256>(A_L1_[((k % 2) * 32768)], A[(((cid / 64) * 2097152) + (k * 256))]);
    }
    {
      auto B_L1_ = B_L1.Get<half>();
      tl::ascend::copy_gm_to_l1<half, layout::RowMajor, layout::zN, 16384, 16384, 256, 256>(B_L1_[((k % 2) * 65536)], B[((k * 4194304) + ((cid % 64) * 256))]);
    }
    ...
    if (k == 0)
      tl::ascend::gemm<half, float, 128, 256, 256, true>(A_L1_, B_L1_, C_L0_);
    else
      tl::ascend::gemm<half, float, 128, 256, 256, false>(A_L1_, B_L1_, C_L0_);
  ...
  {
    auto C_L0_ = C_L0.Get<float>();
    tl::ascend::copy_l0c_to_gm<float, half, layout::RowMajor, 128, 256, 16384, 16384>(C[(((cid / 64) * 2097152) + ((cid % 64) * 256))], C_L0_[0]);
  }
